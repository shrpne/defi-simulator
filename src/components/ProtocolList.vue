...
async function simulate() {
    ...
    const response = await simulateBundleRpc([
        {
            from: userAddress,
    ...
    totalGasUsed.value = response.data.result.reduce((sum: number, tx: any) => {
        return sum + parseInt(tx.gasUsed, 16);
    }, 0);

    assetChanges.value = extractAssetChanges(response.data, userAddress);
